...
concern, the git webhook says why a delivery linked nothing and the settings
page shows the URL GitHub must call, the CRM exports a lead with every field it
holds, `.md` attachments open as documents instead of raw text, the "My leave"
...
`git_links`, deleting duplicate rows first – the oldest of each set stays.
Then redeploy the API and web images (or the single image); the desktop app
updates itself from the release.
