...
- Читання: search, list_projects, list_companies, get_company_overview, list_my_tasks, get_project_status, get_cycle_progress, list_overdue_invoices, get_receivables_aging, list_unbilled_time, find_kb_page. list_projects/list_companies — точка входу для отримання id: решта інструментів приймають projectId/companyId, які агенту більше не треба вгадувати. Відповіді всіх інструментів проходять scrub: службові поля (version, deletedAt, templateSourceId) і секрети (portalToken) до контексту моделі не потрапляють.
- Дії: create_task, update_task_status, assign_task, comment_on_task, log_time, create_invoice_from_time, create_invoice_from_project, send_invoice, record_payment, send_payment_reminder, create_quote, create_note, move_deal, create_kb_page, request_leave, approve_leave, create_job_opening, move_applicant.
