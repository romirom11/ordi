...
Один застосунок з однією базою, який покриває пʼять доменів:

1. **CRM**: компанії, контакти, угоди, нотатки, історія.
...
Джерело правди про клієнтів. Компанія: хаб, від якого розходяться контакти, угоди, проєкти, документи, рахунки.

### 7.2. Компанії

- CRUD; статуси lead → active → paused → archived (перехід вільний, пишеться в activity).
- Картка: шапка (назва, домен з фавіконом, статус, owner, теги-мітки), таби: Огляд / Контакти / Угоди / Проєкти / Рахунки і Quotes (видимі тільки з finance.read) / Нотатки / Файли / Активність.
...
- Kanban за етапами з drag-and-drop; список з фільтрами; сумарні значення й weighted-сума (amount × probability) по етапах.
...
### 7.5. Нотатки й активність

Rich-text нотатки на компанії/контакті/угоді, pinned. Активність: автоматична стрічка з activity_log: зміни етапів, створення проєктів, документи, оплати, нотатки.
...
- Фільтри: статус (і категорія), тип, пріоритет, виконавець, мітки, цикл, дати (прострочені, цей тиждень…), текст. Масові дії: статус, виконавець, мітки, цикл, пріоритет.
...
In-app (дзвіночок, read/unread, перехід до сутності) + email-дублювання за типами з профілю. Типи: згадка, призначення, коментар у моїй задачі, зміна статусу моєї задачі, quote accepted, оплата, прострочення, дедлайн сьогодні, git PR merged у моїй задачі. Digest-режим email (раз на день) як альтернатива миттєвим.
...
core: auth/*, me (профіль+permissions+memberships), users, roles, search, dashboard, dashboards (+widgets, +widget-data), notifications, attachments (presign/register), custom-fields, saved-views, webhooks, audit, stream.
crm: companies (+overview), contacts, deals (+move), deal-stages, notes.
...
- Читання: search, list_projects, list_companies, list_contacts, list_deals, list_deal_stages, list_custom_fields, get_company_overview, list_my_tasks, get_project_status, get_cycle_progress, list_overdue_invoices, get_receivables_aging, list_unbilled_time, find_kb_page. list_* — точка входу для отримання id: решта інструментів приймають projectId/companyId/dealId/stageId, які агенту більше не треба вгадувати. Відповіді всіх інструментів проходять scrub: службові поля (version, deletedAt, templateSourceId) і секрети (portalToken) до контексту моделі не потрапляють.
- Дії: create_task, update_task_status, assign_task, comment_on_task, log_time, create_invoice_from_time, create_invoice_from_project, send_invoice, record_payment, send_payment_reminder, create_quote, create_note, create_company, create_contact, create_deal, move_deal, create_custom_field (потребує settings.manage у скоупі), create_kb_page, request_leave, approve_leave, create_job_opening, move_applicant.
...
Деструктивні операції (delete, cancel) через MCP не виставляються. Всі дії пишуться в activity з actor_type=agent (службовий користувач Agent): у UI завжди видно, що зробив агент, з diff-ом.
