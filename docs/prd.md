...
1. **CRM**: research leads, компанії, контакти, Work-черга, sales activities, playbooks, угоди, нотатки, історія.
...
### 7.4. Leads, Work і research

- Lead належить компанії, може мати запропонований контакт і owner, зберігає product/service pursuit та структурований research: signal, evidence, fit, timing, score, джерела й дату перевірки, suggested channel, opener і caution.
...
- Імпорт research JSON має preview перед записом, спочатку матчує компанії за нормалізованим доменом, дедуплікує leads і зберігає повний batch разом із виключеними кандидатами та причинами.
...
crm: companies (+overview), contacts, leads (+research import preview/commit, +convert), sales-work, sales-activities (+complete/+cancel), sales-message-templates, sales-sequences (+enroll/+stop), deals (+move), deal-stages, notes.
...
- CRM sales loop: `list_leads`, `get_lead`, `create_lead`, `update_lead`, `get_sales_work`, `list_sales_activities`, `schedule_sales_activity`, `update_sales_activity`, `complete_sales_activity`, `cancel_sales_activity`, `convert_lead`, `preview_research_import`, `import_research`, `list_sales_playbooks`, `save_sales_message_template`, `save_sales_sequence`, `manage_sales_sequence`. `import_research` має описану shared-схему payload; `schedule_sales_activity` приймає `templateId` і рендерить placeholders на API; sequence enrollment планує ручні дії, але не надсилає повідомлення.
