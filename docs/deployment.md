...
4. **Сховище файлів**: залиште вбудований MinIO (створіть бакет `ordi` у
   ...
   Для вбудованого MinIO обовʼязково задайте `S3_PUBLIC_ENDPOINT` – адресу
   MinIO, яку бачить **браузер** (наприклад, `https://files.your-domain.com`,
   спроксовану на порт 9000). Presigned-посилання підписуються проти цієї
   адреси; без неї браузер отримує URL на `http://minio:9000` – хост, що існує
   лише в docker-мережі, і аплоуд падає (за https ще й як mixed content). Для
   зовнішніх R2/S3 змінна не потрібна – `S3_ENDPOINT` і так публічний.
...
- [ ] S3/MinIO з бакетом – інакше вкладення недоступні (presign – заглушка).
      Для вбудованого MinIO – `S3_PUBLIC_ENDPOINT` з адресою, доступною з
      браузера (див. §4), інакше аплоуд файлів не працюватиме.
