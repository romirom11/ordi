...
interface ActiveTimer { taskId: string; startedAt: string; elapsedSeconds: number; ref?: string | null; title?: string | null }
...
export function TimerIndicator() {
  ...
  return (
    <div className="flex items-center gap-2 rounded-md bg-primary/10 px-2 py-1 text-xs">
      ...
      <span className="truncate text-muted-foreground" title={timer.title ?? undefined}>{timer.ref ?? 'timer'}</span>
