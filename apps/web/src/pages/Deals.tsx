...
export function DealsPage() {
  ...
  const move = useMutation({
    ...
    onError: (e) => alert(e instanceof ApiError ? e.message : t('deals.moveFailed')),
  });
  ...
  const onMove = (deal: Deal, stageId: string) => {
    ...
    if (target?.isLost) {
      const reason = window.prompt(t('deals.lostReasonPrompt'));
  ...
  return (
    <div className="flex h-full flex-col">
      <PageHeader
        ...
        actions={canWrite && stages.length > 0 && <Button size="sm" onClick={() => setCreating(true)}><Plus size={14} /> {t('deals.newDeal')}</Button>}
      />
      ...
      {isLoading ? (
        ...
        <EmptyState title={t('deals.noStages')} hint={t('deals.noStagesHint')} />
      ) : (
              ...
              <div key={stage.id} className="flex w-64 shrink-0 flex-col rounded-lg border border-border bg-muted/30">
                <div className="border-b border-border p-2.5">
                  ...
                  <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span className="tabular-nums" title={t('deals.weighted')}>≈ {fmtMoney(weighted)}</span>
                  </div>
                </div>
                <div className="flex-1 space-y-2 overflow-y-auto p-2">
                  {stageDeals.length === 0 ? (
                    <p className="px-1 py-4 text-center text-xs text-muted-foreground">{t('deals.empty')}</p>
                  ) : stageDeals.map((d) => (
...
function NewDealModal({ stages, onClose, onCreated }: { stages: Stage[]; onClose: () => void; onCreated: () => void }) {
  ...
  const mut = useMutation({
    ...
    onSuccess: onCreated,
    onError: (e) => setError(e instanceof ApiError ? e.message : t('deals.createFailed')),
  });
  ...
  const submit = (e: FormEvent) => {
    ...
    if (!stageId) { setError(t('deals.stageRequired')); return; }
    mut.mutate();
  };
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-lg border border-border bg-card p-5 shadow-lg" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold">{t('deals.newDeal')}</h2>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {canCrm && (
            <div className="space-y-1">
              ...
              <Select value={companyId} onChange={(e) => setCompanyId(e.target.value)} className="w-full">
                <option value="">{companiesQ.isLoading ? t('common.loading') : t('deals.noClient')}</option>
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground">{t('deals.stage')}</label>
