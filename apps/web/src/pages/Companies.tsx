  ...
  return (
    <div>
      <PageHeader
        ...
        actions={canWrite && (
          <Button size="sm" onClick={() => setCreating(true)}><Plus size={14} /> New client</Button>
        )}
      />
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <EmptyState
            title={q || status ? 'No clients match your filters' : 'No clients yet'}
            hint={q || status ? 'Try a different search or status.' : 'Add your first client to start tracking deals, projects, and invoices in one place.'}
            action={canWrite && !q && !status ? <Button size="sm" onClick={() => setCreating(true)}><Plus size={14} /> New client</Button> : undefined}
          />
        ) : (
...
function CreateCompanyModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  const mut = useMutation({
    ...
    onError: (e) => setError(e instanceof ApiError ? e.message : 'Could not create client.'),
  });
      ...
      <div className={cn('w-full max-w-md rounded-lg border border-border bg-card p-5 shadow-lg')} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button type="submit" size="sm" disabled={mut.isPending}>Create</Button>
          </div>
        </form>
      </div>
    </div>
