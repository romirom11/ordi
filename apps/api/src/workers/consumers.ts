...
import { buildSlackMessage } from './slack-messages';
...
/**
 * Slack notifications. For a subset of business events (see slack-messages.ts,
 * which owns both the list and the copy), post a human message to Slack.
 * Resolution order (Linear-style, webhooks kept as legacy fallback):
...
const slack: Consumer = {
  ...
  async handle(ev) {
    ...
    if (!msg) return; // not a Slack event, or nothing worth posting
    ...
    const res = await fetch(target.url, {
      ...
      // The legacy webhook path posts the same blocks as the bot, so a
      // workspace still on an incoming webhook reads the same message.
      body: JSON.stringify({ text: msg.text, blocks: msg.blocks, unfurl_links: false }),
