...
import {
  ...
  salesActivityCompleteSchema, salesActivityCancelSchema,
...
const ROWS: Row[] = [
  ...
  ['/auth/forgot-password', 'post', 'Request a password reset link (answers the same whether or not the address exists)', null, 'free', 'core'],
  ['/auth/reset-password/{token}', 'get', 'Check a password reset link', null, undefined, 'core'],
  ['/auth/reset-password', 'post', 'Set a new password with a reset token', null, 'free', 'core'],
  ...
  ['/attachments', 'post', 'Upload a file (multipart/form-data: file, optional entityType/entityId; returns a signed embeddable src)', null, 'free', 'core'],
